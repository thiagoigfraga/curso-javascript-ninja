(function (win, doc) {
  ...
  */
  ...
  `$message`.
  */
  let $message = doc.querySelector("textarea[cols='30'][rows='10']");
  /*
  ...
  */
  ...
  $button.addEventListener("click", (event) => {
    ...
    let confirma = confirm("Tem certeza que deseja enviar o formulário?");
    if (confirma) return alert("Enviado com sucesso");
  ...
  */
  ...
  let regex = /^[\w+.]*\@{1}[^\W]+.{1}\w{2,}(?:\.\w{2})?$/gm;

  console.log(regex);
  function isValidEmail(email) {
    let regexResult = regex.exec(email);
